...
function addServiceFromTemplate(businessName_, moduleName_) {
    ...
    if (fs.existsSync(path) === true) {
        vfs.src(temppath + '/src/microservice-template/tax/server/tax-sb/**/*').pipe(vfs.dest(temppath + '/src/' + businessName + '/service/' + serviceName)).on('end', callbackForExist);
    } else {
        vfs.src(temppath + '/src/microservice-template/tax/**/*').pipe(vfs.dest(temppath + '/src/' + businessName)).on('end', callback);
    }
}
